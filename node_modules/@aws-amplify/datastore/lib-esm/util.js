...
import { isPredicateGroup, isPredicateObj, SortDirection, isModelAttributeCompositeKey, } from './types';
...
export var establishRelation = function (namespace) {
    ...
    Object.keys(namespace.models).forEach(function (mKey) {
        ...
        var createCompositeKeysMap = function (compositeKeys, fields) {
            var e_3, _c;
            if (compositeKeys === void 0) { compositeKeys = {}; }
            var _loop_1 = function (field) {
                var rest = fields.filter(function (f) { return f !== field; });
                if (field in compositeKeys) {
                    compositeKeys[field] = __spread(new Set(__spread(compositeKeys[field], rest)));
                    return "continue";
                }
            ...
            try {
                for (var _e = __values(model.attributes), _f = _e.next(); !_f.done; _f = _e.next()) {
                    ...
                    if (isModelAttributeCompositeKey(attribute)) {
                        model.compositeKeys = createCompositeKeysMap(model.compositeKeys, attribute.properties.fields);
                    }
                    if (attribute.type === 'key') {
                        var _g = attribute.properties.fields, fields = _g === void 0 ? [] : _g;
                        try {
